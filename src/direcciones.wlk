...
object izquierda{
	...
	method moverse(unObjeto, nivel) = if( nivel.toString() == "nivel1" ) unObjeto.position().left(2) else unObjeto.position().left(1) 
	method dirOpuesto(unObjeto, nivel) = if ( nivel.toString() == "nivel1" ) unObjeto.position().right(2) else unObjeto.position().right(1)
	
	method moverse(unObjeto) = unObjeto.position().left(2)
	method dirOpuesto(unObjeto) = unObjeto.position().right(2)
}
object derecha{
	method moverse(unObjeto, nivel) = if( nivel.toString() == "nivel1" ) unObjeto.position().right(2) else unObjeto.position().right(1)
...
object abajo{
	method moverse(unObjeto, nivel) = if (nivel.toString() == "nivel1" ) unObjeto.position().down(2) else unObjeto.position().down(1)
...
object arriba{	
	method moverse(unObjeto, nivel) = if ( nivel.toString() == "nivel1" ) unObjeto.position().up(2) else unObjeto.position().up(1)
