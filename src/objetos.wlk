...
class Caja inherits Posicion (soyUnaCaja = true) {
	...
	method image() = if (self.estoyEnMeta()) {
		resolucion + "/" + cajaEnMeta
	} else {
		resolucion + "/" + stringDeObjeto
	}

	method text() = if (configuraciones.nivelActual().soyUnNivelHardcoreTime()) {
		self.modoHardCoreTimer()
	} else {
		""
	}

	method modoHardCoreTimer() = if (!yaEstubeEnMeta) {
		"+" + self.hardCoreTimeBonificacion().toString()
	} else {
		""
	}
	...
	method contador() {
		if (configuraciones.nivelActual().soyUnNivelPuzzle()) configuraciones.elcontadorDePasos().incrementar()
...
class Oveja inherits Caja {
	...
	override method image() = if (!self.estoyEnMeta()) {
...
class CheckPointStart inherits Checkpoint {
	
	override method hacerAlgo(direccion) {
		configuraciones.configStopMusic()
		game.clear()
		if(nivel0.nivelIntroductorio()){
			nivel0.cargarNivel()	
		}
		else{
			siguienteNivel.cargarNivel()
...
class Meta inherits Pisable(tipo = 1) {
	...
	const property clave = [self.position(),tipo]
	
	method passwordInfo()="["+position.x().toString() + ","+ position.y().toString() +"]" + "tipo: " + self.tipo().toString()
...
class Cronometro inherits Estatico(position = game.at(12, 12)) {
	...
	method text() = if (!self.nivelCompletado()) {
		self.segundos().toString()
	} else {
		"YEAH!"
	}
...
class ReadyYGO inherits Estatico {
	...
	override method position() = if (self.ready()) {
		game.at(10, 6)
	} else {
		game.at(11, 6)
	}

	method image() = if (self.ready()) {
		"menorResolucion/ready.png"
	} else {
		"menorResolucion/go.png"
	}
